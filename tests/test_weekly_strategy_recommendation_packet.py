    ...
    def test_build_packet_combines_own_and_competitor_signals(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            with patch.object(weekly_strategy_recommendation_packet, "_now_local", return_value=datetime.fromisoformat("2026-04-16T09:00:00-04:00")), patch.object(
                ...
                self.assertIn("strategy_frames", payload)
                self.assertIn("stable_patterns", payload["strategy_frames"])
                ...
                meme_guidance = next(item for item in payload["social_plan"]["lane_guidance"] if item["lane"] == "meme")
                ...
                self.assertIn("## Lane Guidance", markdown)
                self.assertIn("## Stable Patterns To Keep", markdown)
                self.assertIn("## One-Time Experiments", markdown)
                ...
                self.assertIn("Operator: Run Meme Flow", markdown)

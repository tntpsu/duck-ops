...
### 1. ~~The printable bundle~~ → parts route SHIPPED 2026-09-15 (Surface 71); next: print one
- Operator: "Can u do the power ranger first?" → Power Ranger and Highland Cow both `ready`: `<run>/paint_to_print_handoff/parts_bundle.3mf` (one object, one part per filament slot, 57 mm) + `handoff_qa_board.png`. Next: open in Bambu Studio, confirm the slot map, print one; then the surface-graphics layer (relief details the generator did not split), `concept_to_print_status.json` registration, and the Studio 3D Lab wiring.

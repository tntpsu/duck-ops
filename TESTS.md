...
**Scope note (honest):** Library is actively wrapped; the helper is **available** (CSS+JS injected) on the other standalone SPA pages (Desk, Decisions, Reviews, Workflows, OS, Studio×3) but their loads aren't yet routed through it — those pages paint into many small DOM targets (`focusTitle`, `decisionTitle`, `reviewTitle`, panels) with no single content container, so each needs a per-page container strategy. Deferred as a per-page fast-follow rather than risk refactoring 5 working pages in one change. Server-rendered intel pages need no wrap (no client data load). **Tier:** read-only UI; viewer bounce to load the new code is a local service restart.

...
**2026-09-24 — unmeasured posts were being scored 0.0 (operator: "are we learning anything from recent posts to continue to get better?").** `_engagement_score` read `post["metrics"]`, which a failed fetch leaves as `{}`, and summed it to `0.0` — indistinguishable from a post that was measured and genuinely got zero. `_rollup_rows` then averaged those fabricated zeros into the lane mean. Live effect: 9 of meme's 15 posts were `metric_status=fetch_failed`, so the lane reported **1.60 and ranked last** when its measured average was **4.00 and ranked first**; the published belief "duckvideo is the current strongest workflow" and the logged change "Top workflow moved from `jeepfact` to `duckvideo`" were both artifacts. Those beliefs feed `ideas_to_test` → `product_concept_queue` → design briefs, so the error was steering generation. The collector already recorded `metric_status` per post and counted it in `metric_status_counts`, and the **writeback path had guarded on exactly this status since June** (`::test_skips_on_fetch_failed_metric_status`) — only the rollups never got the guard. Sibling of `feedback_plausible_fallbacks_mask_failure`: absence rendered as a plausible number, not an error.

Fixed with one shared definition of "measured" (`MEASURED_METRIC_STATUSES` / `_is_measured`, now also backing `metrics_coverage_pct`), `avg_engagement_score: None` for a lane with nothing measured, `measured_post_count` on every rollup row and in every belief's evidence string ("6 of 15 posts measured"), and unmeasured lanes sorting below a genuine measured zero.

Root cause of the 9 failures, from `metric_errors` in state: 6 × `(#100) Tried accessing nonexisting field (permalink_url)` — Graph 400s the whole request over one unsupported field, discarding the reaction and comment counts it would have returned — and 3 × `Object with ID '1222…' does not exist`, a bare page-post id where the working posts use `{page_id}_{post_id}`. `_fetch_facebook_metrics` now walks a candidate ladder (bare id then page-qualified via `META_PAGE_ID`, each with full then core field set) and records the recovered-from errors.

**Residual (tracked, not prose-only):** the engagement-score rollup is still blended across platforms, so a lane's mean partly reflects its platform mix — `duckvideo` and `review_carousel` have zero Facebook posts while `meme` is 60% Facebook, and measured Facebook posts genuinely score 0–1 with no reach available at all. The new `by_workflow_reach` / `by_theme_reach` rollups are within-platform and carry the cleanly-separating signal (Instagram: duckvideo 179.3 avg reach vs review_carousel 25.8, Welch t=3.92). Splitting the score rollup by platform too is the follow-up. Related structural limit: `window_days` is a hard-coded rolling 30, so at ~1 post/day the sample is pinned near 30 and lane differences will keep failing to separate.
...
| IG metric names per media type (`_fetch_instagram_metrics`) | ✅ `FetchInstagramMetricsTests::test_reels_requests_unified_views_metric_not_video_views` — REGRESSION 2026-08-18: `video_views` is rejected for REELS with a metric-enum 400, so the first Reel's views were silently dropped into `metric_errors`; `views` (Meta's unified metric) live-probed OK (262 views where video_views errored) | n/a | covered by existing per-metric error-append (bad metric degrades to `metric_errors`, other metrics still collected) | n/a | n/a | n/a |
| Lane means exclude unmeasured posts (2026-09-24) | ✅ `test_social_performance_collector.py::UnmeasuredPostsAreNotZeroTests::test_failed_fetches_do_not_drag_the_lane_mean`, `::test_top_workflow_belief_ignores_unmeasured_posts` (the belief that actually shipped wrong) | n/a | ✅ `::test_lane_with_nothing_measured_reports_none_not_zero`, `::test_unmeasured_lanes_never_outrank_measured_ones` | n/a | n/a | ✅ `::test_a_measured_zero_still_counts` (a genuine 0 is data and must stay), `::test_per_post_score_is_none_when_never_measured` |
| Facebook object fetch recovery (2026-09-24) | ✅ `FacebookFetchRecoveryTests::test_retries_without_permalink_url_on_field_error` | n/a | ✅ `::test_gives_up_cleanly_when_every_candidate_fails` (still `fetch_failed`, empty metrics, errors recorded) | n/a | ✅ `::test_retries_with_page_qualified_id_when_bare_id_is_unknown` | n/a |
| Reach ranked within platform (2026-09-24) | ✅ `ReachRollupTests::test_reach_rows_are_grouped_within_platform` | n/a | ✅ `::test_posts_without_reach_are_skipped_not_zeroed` | n/a | n/a | ⚠️ residual:score-rollup-still-platform-blended — see background |
...
**2026-08-14 addendum — third sibling of the same class (dry-run branch + selector fragility).** A green dry-run fill (126 chars staged via the stable `li[data-review-region]` selector, submit enabled) was marked `failed` because `capture_target_review_screenshot` still targeted only the ephemeral injected `data-openclaw-target-review` marker, which Etsy's React re-render drops after the fill's input/change events. The June fixes hardened the SUBMIT branch's evidence capture; the DRY-RUN branch (`run_dry_run_fill`, executor:2748) was never wrapped. Fix (commit 947fec7): the screenshot helper itself now ladders marker → transaction-id row → full viewport and never raises, and both executor call sites pass the transaction id — no caller can be killed by evidence capture again. `failure_class=unexpected_executor_failure` is NOT in `RECOVERABLE_FAILURE_CLASSES`, so the item needed a manual production `queue` requeue. Regression: `test_review_reply_discovery.py::CaptureTargetReviewScreenshotTests` (3 tests: fallback ladder, never-raise, viewport fallback).
...
| operator match overrides (`config/product_model_overrides.json`, keyed by model dir NAME so decisions survive the daily rescan; 2026-08-17 operator: reject spa↔Baby Yoda, confirm captain↔Pilot) | unit: reject→`operator_rejected` unmatched; confirm→`needs_review:false` `matched_by:operator_confirmed` | unit: confirm target missing from catalog → fails closed to `operator_confirm_target_missing`, never re-matched elsewhere | unit: broken JSON degrades to no overrides (flags survive) | n/a | n/a | n/a | conftest patches `PRODUCT_MODEL_OVERRIDES_PATH` in BOTH repos (read-isolation: tests must not see real operator decisions). NOTE: rejecting one job dir let sibling `generalization_spa_duck_v1` re-match the same product next scan — enumerate sibling dirs when rejecting ([[feedback_incomplete_fix_enumerate_all_branches]]) |
...
**Regression row (2026-08-19, operator: "top 5 ducks… but only showed 4"):** the flat `MONTAGE_MAX_SLIDES=8` cutoff silently dropped duck 5 of a 5-duck roster (5×2 photos = 10 candidates) while the hook, baked at prepare from the selection, still promised 5 — a [[no-silent-caps]] violation. Fix: photos-per-duck budget (`MONTAGE_MAX_SLIDES // len(segments)`, min 1) so every selected duck always appears, with a log line when trimming. Pinned by `test_duck_video_helper.py::TestMontage::test_every_duck_appears_when_roster_exceeds_slide_cap`.

**Sibling silent cap, same day (operator: "why didn't Penn State make the list?"):** `phase1_observer.match_catalog` stored only `exact_matches[:5]` in dict-iteration order — "football team duck" fully matched 8 college ducks but the arbitrary 5 won permanently, so Penn State + both Florida Gators could NEVER rotate into a football-trend montage (`selection._trending_candidates` reads the stored list). Fix: `CATALOG_MATCH_MAX=10`, title-sorted for determinism, log line on truncation. Pinned by `duck-ops/tests/test_phase1_observer_catalog_match.py` (2 tests). Note `catalog_match.find_existing_matches` has the same `max_matches=5` shape but its consumers (concept dedup) genuinely want top-N — left as-is.
...
## Surface 64c — duckvideo Track B: AI motion video (Veo image-to-video) (SPEC-FIRST 2026-08-18, operator: "i thought we would do rotating videos with glb, fade in and out videos with photos and then working a path to have AI make videos if they have a pic of our duck" → "Looks good")

**Design:** third mode `ai_motion` in the weekly rotation (ISO week % 3: single→montage→ai_motion; env `DUCKVIDEO_MODE` proposes, data disposes — rephases which weeks get montage vs the old even/odd split, cosmetic). Input = real Shopify CDN photo + concept from versioned `config/duckvideo_motion_concepts.json` (curated library, AI executes but never invents — free-form per-run brainstorming is the Thursday-lane ~50%-unexecutable failure mode); concept×product pairs never repeat within `dedup_window_days` (source: `runs/*/state_duckvideo.json` — receipts REJECTED as dedup source: publish-only subset misses operator-skipped drafts). Veo via google-genai 1.68.0 (`GEMINI_KEY`, gemini_helper auth pattern); 9:16 720p → ffmpeg-normalized to the lane contract (1080x1920 h264 yuv420p +faststart + audio — Veo 3.1 on the Gemini API generates audio natively; `generate_audio` and `person_generation=dont_allow` are Vertex-only/rejected params, live-probed 2026-08-18). LIGHT gate (human reviews every output): ffprobe validation + spot frames at 0.5/3/6.5s attached to the review email. Hard caps in the library config (1 generation/run, 8s). Kill switches: `DUCKVIDEO_MODE=single`, empty concepts file. Model ids DRIFT (3.0-fast retired by 2026-08-18 — preflight caught it): `DUCKVIDEO_VEO_MODEL` env overrides without a deploy.

| Use case | Happy | Starved/dedup-exhausted | Veo fails/times out | Validation fails | Mode/env override | Old-run migration | Test pollution |
|---|---|---|---|---|---|---|---|
| concept library + rotation (`duck_video_motion_helper`) | unit: deterministic pick, prompt slots+guardrails filled | unit: all pairs used → None → single fallback + `ai_motion_unavailable` reason | n/a | n/a | n/a | unit: state files without `motion` key ignored by `motion_used_pairs` | conftest autouse redirects `MOTION_CONCEPTS_PATH` + `RUNS_DIR` (read-isolation) |
| Veo generation (`generate_motion_video`) | **gated eval `scripts/eval_duckvideo_motion.py` PASSED live 2026-08-18**: 54s generation, all probes green, spot frames visually verified (knit texture rendered as yarn vs printed plastic — flagged, review gate's call) | n/a | unit: poll timeout / operation.error / SDK exception → `status:"failed"`, never raises; unit: seconds-cap refusal makes NO api call; unit: missing GEMINI_KEY fails closed | n/a | unit: env override in choose_mode; model-id preflight in eval | n/a | genai client monkeypatched in all units |
| normalize + validate (`normalize_motion_video`, `validate_motion_video`) | eval: live clip probed h264/1080x1920/duration/audio/size | n/a | unit: ffmpeg missing fails soft | unit: issue table (codec/dims/duration/audio/size); flow treats issues as generation failure → kenburns + `ai_motion_failed: validation:` reason | n/a | n/a | n/a |
| flow + email + review frames (`steps.py` ai_motion branches) | unit: motion state keys (concept_id/version/prompt) + product compat key; email mode line + frame attachments; publish untouched (existing 64/64b tests green = contract proof) | unit: prepare starved → single + reason in state | unit: render fail → kenburns + `ai_motion_failed:` fallback_reason (existing steps.py fallback path) | unit: validation issues → kenburns fallback | unit: choose_mode week%3 table + all-modes coverage + env override | n/a | n/a |
| cost control | config-capped: max_generations_per_run=1 (single generation per prepare by construction), 8s hard cap enforced pre-call (unit) | n/a | n/a | n/a | n/a | n/a | n/a |
| first live ai_motion reel | ⚠️ manual:supervised (Tier-3): next ai_motion week (or DUCKVIDEO_MODE pin) → flow → approve by reply → sidecar posts → receipt + cards green, dated entry here | n/a | n/a | n/a | n/a | n/a | n/a |

**Build status (2026-08-18): SHIPPED same day.** Helper + library config + 17 unit tests; gated eval passed live on first real generation (Knitted Duck × trail_ride_v1 — eval preflight caught the retired 3.0-fast model id, then two Gemini-API param rejections (`generate_audio`, `dont_allow`), all fixed before wiring per the risk-order discipline); flow wiring (selection mod-3 + prepare/render/email branches + transition concept_id) + 8 flow tests + 3 selection tests, full duckvideo suite 56 green. Lane-health cards need NO change (never branch on renderer — verified). Eval clip + frames sent to operator for the fidelity verdict.

**Operator round 2 (2026-08-18, same day): both review findings fixed + re-eval'd.** (1) Restyle — "it made it look like a knitted duck (like its name)": the product NAME in the prompt got read as a material instruction. Fix: name never enters the prompt (`build_motion_prompt` pins this, `test_build_prompt_keeps_product_name_out`); identity comes from the photo. (2) "The first part was just the image" — top-level `image=` is FIRST-FRAME anchoring → clip opens on the studio photo and crossfades. Fix: `reference_images=[VideoGenerationReferenceImage(reference_type="ASSET")]` inside `GenerateVideosConfig` — appearance guidance from frame one, no anchor. Two more live-probed Gemini-API constraints recorded: `reference_images` is a CONFIG field (top-level kwarg → TypeError), and `negative_prompt` is rejected in the reference-image use case (constraints ride in prompt text; config library v2). Re-eval: duck reads as glossy printed plastic w/ knit texture matching the real product, scene from frame one, all probes green.

**Round 3 (2026-08-22, operator: "the video added duck feet to the Michigan Duck"):** the reference image guides appearance but Veo still COMPLETES from its cartoon-duck prior — it invented webbed feet absent from the product. The prompt forbade changes but not additions; `build_motion_prompt` now bans added feet/legs/limbs/accessories ("base and silhouette stay exactly as shown"), pinned in `test_build_prompt_keeps_product_name_out`. Library v5 adds two operator-suggested weather concepts (rainy_driveway ASMR, tornado_watch chaos-outside/peace-inside). Known residual: Veo can blend the SUBJECT's theme into the scene (cow-print tornado funnel) — content-level, caught by the human review gate by design.
...
## Surface 66 — Operator email surface overhaul (SPEC-FIRST 2026-09-07, operator: "look at the latest emails… are they useful? dynamic? I need a deep evaluation" → "Please do those 6. Also I feel the SEO review is weak and seems to just catch some titles")

Origin: every automated email of the last 30 days was read AS DELIVERED (Gmail `from:me to:me`). All lanes were mechanically green; every one had a visible defect the operator had never reported. Six fixes approved plus an SEO-audit depth question.

Root causes found (each maps to a row below):
- **66.1 fold black hole** — `email_cadence_gate` returned `folded_into_monday_business_digest` for `shopify_seo`, but `business_monday_digest.build_digest_sections` had no SEO section → the 23-item `weak_title` batch was silently dropped on 2026-07-06 AND 2026-08-31 (memory [[fold-defer-is-a-handoff-contract]]). Compounding: (a) `.env` load ordering made the gate fold in the reply-handler process but not in the launchd kickoff; (b) `shopify_seo_kickoff` always restarted the category cursor at index 0, so a 1-item `long_title` batch blocked 92 items in `weak_title`/`weak_description` for weeks; (c) the `high_severity_issue_count` bypass read `candidates`/`top_actions` while the payload key is `items` (dead bypass); (d) audit freshness check missed by 18 min every week (07:35 kickoff vs 07:53 audit stamp, `> 7.0d`); (e) kickoff reported `status: emailed` even when deferred.
- **66.2 review template** — 2★ private draft prompt said "Express that 5-star reviews are important" (Etsy review-manipulation policy risk); daily email told the operator to copy-paste 5★ replies the auto-reply lane posts anyway; empty `Image URL:`; `tmp*.png` attachment. The plaintext is machine-parsed by duck-ops `phase1_observer` regexes (the ONLY feed for `reviews_reply_private`) — zero tests protected that contract.
- **66.3 digest** — flagship weekly email was 4 all-green count lines, no decisions, dead "0 learnings" section.
- **66.4 competitor report** — `_load_previous_competitor_report` picked the daily `_snapshot_only` stub by mtime (+165/+0 artifact); `created_date = datetime.now()` (all 165 "created" on report date); plaintext radar uncapped (165 rows, ~80 from one shop); MOMENTUM NOW sorted by `engagement_delta_7d` where `delta_source == "new_listing"` copies ALL-TIME views into the 7d delta and guarantees `sold_last_7d is None` (top 5 all "—"); hardcoded "20 Top Recommendations" vs 10 rendered; already-made ducks recommended back.
- **66.5 OpenClaw log emails** — `digest`/`trend_digest`/`phase_readiness` bypass the cadence gate entirely (no policy, no operator toggle); Phase Readiness counted trend-ranker rows as "quality-gate decisions" (5247 vs 12) and its subject week was `%W` while the artifact used ISO week (always one week behind).
- **66.6 cross-lane evidence** — trends digest printed `new_items` and `pending_items` (identical) twice; `confidence` saturates at 0.58 for every single-source item (`confidence_cap`); design-brief `_merge_duplicate_themes` kept the OLDEST row on score ties (strict `>`), so `female nurse duck` (sold_7d 14, 62 fresh gap rows) was demoted to `watch` by a 175-day-stale sibling while Soldier Duck shipped with no numbers; `_design_brief_signal` sliced 6 evidence lines to 5, dropping `7d revenue`/`Source refs`.
- **SEO audit depth** (operator's instinct confirmed): 12 rules over exactly two strings (`seo.title`, `seo.description`); title band 35–70 (Google truncates ~60: 74/260 products pass at 61–70), description band 70–165 (industry 120–160: 23 products outside); generator writes into the same band the audit grades ("grading its own homework" → 192/260 products zero issues); `status` fetched but never used (drafts/archived audited + emailed); no image alt, body, handle, structured-data, duplicate-description, or demand-aware checks. 93 actionable resources existed (68 products) while the lane served 1-item batches.

## Use case × failure mode

|  | Happy | Silent drop / stale | Wrong population / count | Contract drift | Policy / safety |
|---|---|---|---|---|---|
| 66.1 fold → digest handoff | `tests/test_business_monday_digest.py::TestFoldContract::test_every_folded_surface_has_a_digest_section` (+ runtime `fold_contract_gaps()` stderr line) | `test_shopify_seo_lane_cursor.py::CursorTests::test_deferred_run_is_resent_on_monday_not_rebuilt`; `test_business_monday_digest.py::TestDigestSections::test_decisions_section_flags_stale_deferred_batch` | `test_shopify_seo_lane_cursor.py::BypassAndResendTests::test_high_severity_bypass_reads_items` | `test_business_monday_digest.py::TestFoldContract::test_shopify_seo_is_not_a_folded_surface` (approval lanes with reply verbs are never folded); `BypassAndResendTests::test_resend_regates_and_marks_awaiting_review` | n/a |
| 66.1 category cursor | `test_shopify_seo_lane_cursor.py::CursorTests::test_cursor_advances_past_last_category` | `CursorTests::test_cursor_wraps_to_start_when_tail_is_empty` | `CursorTests::test_kickoff_status_mirrors_payload_status` | `AuditFreshnessTests::test_audit_stale_at_six_and_a_half_days` | n/a |
| 66.2 low-rating draft | duckAgent `tests/test_reviews_low_rating_followup.py::test_clean_llm_output_is_used_verbatim` | `::test_llm_failure_falls_back_without_review_language` | n/a | `::test_plaintext_still_matches_duck_ops_parsers` (pinned copies of `THANK_YOU_MESSAGE_PATTERN` / `PRIVATE_REVIEW_PATTERN`) | `::test_review_solicitation_in_llm_output_falls_back_to_clean_template`, `::test_issue_detector_catches_solicitation_variants`, `::test_fallback_templates_are_clean` |
| 66.3 digest content | `test_business_monday_digest.py::TestDigestSections::test_profit_section_has_week_over_week`, `::test_decisions_section_lists_open_seo_batch`, `::test_build_section_cross_references_trend_and_competitor`, `::test_competitor_section_skips_snapshot_stub` | `::test_sections_fail_soft_when_state_missing` (updated for the 9-section set) | `::test_learnings_section_omitted_when_empty` | n/a | n/a |
| 66.4 competitor report | duckAgent `tests/test_competitor_report_hygiene.py::test_previous_report_skips_snapshot_stubs`, `::test_previous_report_excludes_current_date` | `::test_radar_created_date_uses_listing_created_ts` | `::test_momentum_now_excludes_new_listing_deltas`, `::test_recommendations_exclude_already_made_and_header_count_matches_rendered`, `::test_html_header_recommendation_count_is_derived` | `::test_plaintext_radar_is_capped_with_per_shop_rollup` (no-silent-caps: dropped count + per-shop rollup line) | n/a |
| 66.5 log emails | `test_notifier_log_emails.py::LogEmailGatingTests::test_digest_kinds_route_to_off_policies`, `::test_operator_can_turn_a_log_email_back_on` | `LogEmailGatingTests::test_truly_unmapped_kind_returns_none` (+ `test_notifier_cadence_gate.py` unmapped fixture moved `digest`→`urgent`); `test_email_cadence_gate.py::test_all_surfaces_registered` pins the 12 surfaces | `test_notifier_log_emails.py::PhaseReadinessTests::test_counts_only_quality_gate_rows`, `::test_subject_week_matches_artifact_week` | config `review_reply_execution.json::auto_drain_send_session_summary=false` (portal `/portal/reviews` parity; `test_notifier_whatsapp` now stubs the gate) | n/a |
| 66.6 cross-lane evidence | `test_product_concept_merge_and_ip.py::MergeTieBreakTests::test_merge_tie_prefers_fresher_row`, `::test_higher_score_still_beats_freshness`, `::test_design_brief_signal_keeps_all_evidence_lines` | `test_trend_ranker_digest.py::TrendDigestTests::test_pending_section_excludes_new_items` | `TrendDigestTests::test_digest_hides_saturated_confidence`, `::test_html_card_and_stats_count_the_same_population` | duckAgent `creative_agent/runtime/tests/test_design_brief_prompt_evidence.py::test_design_brief_why_now_must_cite_numbers` (light gate — human reviews every brief) | `test_product_concept_merge_and_ip.py::GreekLetterIpTests::test_greek_letter_organization_is_ip_sensitive`, `::test_single_greek_word_without_org_context_is_not_blocked` |
| SEO audit scope | `test_shopify_seo_audit_status.py::DraftExclusionTests::test_draft_and_archived_products_are_excluded` | n/a — threshold tightening (title ≤60, description 120–160, duplicate descriptions, handle quality, alt text) is **Phase 2**: it must move the generator's target band in the same commit or the lane loops on its own output ([[feedback_llm_detection_vs_generation]]); gate via `scripts/eval_shopify_seo.py` | n/a | n/a | n/a |

Residual (tracked, not fixed here): Phase Readiness has no portal surface (email now off by default via `/portal/workflows`; artifact still written); SEO threshold tightening (Phase 2 above); trend `confidence` remains uncalibrated (`# no real outcome loop yet`) — hidden from the operator, not fixed.
...
| scheduler_health stale missed_run verdict (sidecar grades 06:xx, next re-grade 12:00) | `test_scheduler_health.py::test_healthy_job_has_no_verdict_deadline` (no deadline on healthy) | duckAgent `creative_agent/runtime/tests/test_viewer_scheduler_verdict_expiry.py::test_verdict_still_valid_is_served_unchanged` | `::test_expired_verdict_is_recomputed_on_read` (0.1s live rebuild), `::test_expired_verdict_with_failed_recompute_is_downgraded_not_red` | `test_scheduler_health.py::test_missed_job_detects_expected_start_without_log_event` pins `verdict_valid_until = next_expected_at + grace` on item AND payload | `::test_payload_without_deadline_is_untouched`, `::test_missing_file_is_empty_dict` |

## Surface 68 — Concept-image taste calibration + style-memory tiers (2026-09-09, operator: "i almost every day ask chatgpt to make me a concept and it does well… make something into a rubber duck, then take off details for 3d printing")

Origin: the concept→image→3D lane produced zero printable ducks in 108 days; its one attempt (2026-07-06) failed QA as a cow head on a car. Root causes: (a) the style-memory keyword matcher attached the Race Car Duck *catalog photo* as a generation reference for "Highland Cow Jeeping Tradition Vehicle Ducking…"; 46 of 62 "approved" style examples were auto-ingested catalog photos nobody judged; (b) the QA judge's vocabulary hard-failed traits the operator actually approves (mascot head replacing the duck head, arms holding a prop, headless designs) — 14 of 21 historical QA verdicts were fails, several for exactly this. Six operator-approved ChatGPT ducks (power ranger, nittany lion, abu, iago, jason, headless horseman) now define the taste: BODY stays a squat flat-bottomed duck with tail; HEAD is free (helmet/mask/mascot/animal head/none); chunky molded arms with one broad prop are fine; 4–6 flat colors.

Shipped: `helpers/duck_style_system.py` tiers (`operator_pick` / `curated_product` / `catalog_reference` / `lane_generated`, inferred from source, stored explicitly) + `prompt`/`traits` fields; `scripts/duck_style_memory.py --tier/--prompt/--traits`; `data/duck_style_memory.json` backfilled (68 approved: 15 operator_pick, 7 curated, 46 catalog) with the six ChatGPT ducks (images in `runs/style_lab/operator_chatgpt_20260909/`); `policies/creative_memory.json` v2 rubber_duck_base rules; `creative_policies.py` judge prompt + `_APPROVED_TASTE_FRAGMENTS` softening; `docs/current_system/DUCK_STYLE_LAB.md` tier table. NOT yet shipped (next slice): generation stops attaching catalog-tier images, GPT Image two-turn recipe, golden eval.

## Use case × failure mode

|  | Happy | Wrong population (catalog photo treated as judgment) | Judge over-rejects operator taste | Contract drift | Regression |
|---|---|---|---|---|---|
| Style-memory tiers | duckAgent `tests/test_duck_style_system.py::test_normalize_keeps_tier_prompt_and_traits` | `::test_normalize_infers_tier_from_source_and_rejects_unknown_tier` (catalog_training→catalog_reference, unknown source→catalog_reference, lab→operator_pick) | n/a | unknown tier raises (same test) | existing `::test_style_memory_upsert_moves_example_between_buckets` still green |
| QA judge calibration | `creative_agent/runtime/tests/test_creative_policies.py::test_judge_prompt_states_head_is_free_and_body_is_duck` | n/a | `::test_mascot_head_replacing_duck_head_is_review_not_fail`, `::test_chunky_arm_holding_prop_is_review_not_fail_even_with_concrete_contract` | `::test_vehicle_body_and_separate_feet_still_hard_fail` (vehicle body + standing mascot body on separate feet remain hard fails) | 🟡 manual: rerun the 2026-07-06 archived run through `build_visual_policy_qa` once generation is rewired (still expected to FAIL on "vehicle body") |
| Generation references from catalog tier | duckAgent `tests/test_duck_style_system.py::test_style_reference_resolver_skips_catalog_tier_images` | same test (catalog row with an image is NOT resolved; operator pick IS) | n/a | `allowed_tiers` default = operator_pick only | existing `::test_style_reference_resolver_copies_local_memory_images` |

### 68b — Operator two-turn recipe (SPEC-FIRST 2026-09-09, operator: "i just want you to build this robustly")

|  | Happy | Provider/turn failure | Prompt contract drift | Wrong inputs reach the model | Regression |
|---|---|---|---|---|---|
| Turn-1 prompt (`duck_tools/concept_recipe.py`) | `creative_agent/tools/tests/test_concept_recipe.py::test_turn1_prompt_is_short_and_subject_first` | n/a | `::test_recipe_json_is_versioned_and_complete`, `::test_turn1_prompt_caps_words_and_traits` | `::test_clean_subject_strips_marketplace_boilerplate` ("Highland Cow Jeeping Tradition Vehicle Ducking…" → "Highland Cow") | n/a |
| Two-turn generation (`duck_tools/media.py::_generate_with_operator_recipe`) | `::test_operator_recipe_runs_turn1_then_edit_turn` (openai first, edit uses turn-1 file, result carries both prompts/models) | `::test_operator_recipe_keeps_turn1_when_edit_fails` (turn2_status=failed, error recorded, final image = turn 1); `::test_operator_recipe_falls_back_to_gemini_when_openai_unavailable` | `::test_operator_recipe_honors_explicit_gemini_provider` | `::test_operator_recipe_never_attaches_reference_images` | legacy path untouched: existing `test_tools.py` concept-image tests |
| Task wiring (`tasks.py::_run_concept_bundle`) | `creative_agent/runtime/tests/test_concept_image_request.py::test_request_uses_recipe_and_brief_traits` | n/a | `::test_legacy_recipe_keeps_old_kwargs` | `::test_recipe_request_has_no_reference_paths` | schema default `image_recipe="operator_two_turn"` for every existing input |
| Real-generation eval (`scripts/eval_concept_image.py`, manual, spends credits) | 🟡 manual: control (legacy) vs treatment (recipe) on `tests/fixtures/concept_image_golden.json`; gate ≥80% QA pass and 0 body-drift fails; contact sheet reviewed by eye ([[feedback_visual_output_needs_eyes]]). **Control 2026-09-09 (legacy prompt, Gemini): judge v1 8/20 pass, judge v2 16/20 pass, 1 body drift. Treatment 2026-09-09 (recipe v1): OpenAI credits ran out at 12:16 mid-run — cases 1–10 on GPT Image, 11–20 fell back to Gemini, judge v2 could not score (429 insufficient_quota). BY EYE against the operator's references: 18/20 acceptable, 0 body drift; misses = Nittany Lion lost its scarf/face to the turn-2 'take off details' wording (fixed in recipe v2), breast-cancer ribbon missing. Operator review of the same sheet (2026-09-09): all acceptable except the Nittany Lion (a generic lion: the operator's real prompt said *mascot*), and the Power Ranger has no beak (operator: sometimes fine, e.g. the Nittany Lion mascot) → recipe v3 + judge `beak` field, row below. Formal judge-v2 gate re-run: see 68c (2026-09-11, PASS 19/20).** | recorded per case (`records.jsonl` incremental) | recipe_version stamped in every receipt; `--rejudge DIR` re-scores an existing run with the current judge without regenerating | n/a | operator's six ChatGPT ducks rendered side-by-side as the bar |
| OpenAI quota outage (2026-09-09 12:16, 50 of 186 calls failed `insufficient_quota`, no card went red) | 🔴 OPEN: no OS card watches provider quota/429s; `llm_cost_dashboard` reads a once-a-day summary. Next: cheap reader over the tail of `llm_call_log.jsonl`, red when quota errors appear in the last 3h, loader + registration tests ([[feedback_loader_and_registration_both_need_tests]]) | — | — | — | — |
| Judge v2 — structured verdict (`creative_policies._normalize_structured_review`, gpt-4o default) | `test_creative_policies.py::test_structured_headless_and_helmet_heads_pass`, `::test_judge_prompt_defines_structured_fields` | `::test_structured_not_duck_body_fails_even_when_model_said_pass`, `::test_structured_separate_legs_and_real_logo_fail` | `::test_structured_duck_body_overrides_free_form_non_duck_finding` (free-form "non-duck product body" on a bear-headed duck no longer fails), `::test_structured_unnamed_logo_goes_to_review_but_named_mark_fails` (model echoed the must_avoid phrase as a "logo") | `::test_structured_thin_parts_or_missing_traits_go_to_review` | `::test_legacy_free_form_review_still_uses_v1_rules` (reviews without structured keys keep v1 softening) |
| ~~Beak rule~~ RETIRED 2026-09-12 (operator: "the add a beak thing is hurting this. I don't think we need to force that"; the always-on sentence contradicted "no head" and drew the Headless Horseman with a hat). Recipe v5: only subject-triggered head sentences (mascot/animal head, headless: `test_concept_recipe.py::test_turn1_prompt_adds_a_head_sentence_only_when_the_subject_asks_for_it`); judge records the beak, never flags it (`test_creative_policies.py::test_structured_beak_is_recorded_but_never_changes_the_verdict`). Original row (recipe v3, judge `beak` field; operator 2026-09-09: "the power ranger one just doesn't have a beak (but sometimes its ok to not have a beak like the nittany lion mascot)", "the normal prompt said the penn state nittany lion mascot as a rubber duck") | `creative_agent/tools/tests/test_concept_recipe.py::test_turn1_prompt_keeps_the_beak_unless_the_head_is_a_full_mascot` (beak sentence by default; full-head sentence when the subject or keep traits name a mascot/animal head, markers in `full_head_markers`); `test_creative_policies.py::test_structured_missing_beak_goes_to_review_unless_the_head_is_a_full_mascot` | n/a | `::test_judge_prompt_defines_structured_fields` pins the `beak` field and rule text; golden v2 `nittany_lion.subject` = "The Penn State Nittany Lion mascot" (changelog in the fixture); style-memory `prompt` corrected to the operator's literal wording | judge decides in code: `beak=absent` + `head=character_head|none` → pass; + `head=duck|helmet_or_mask|unclear` → needs_review, never fail; unanswered `beak` → no effect on existing verdicts | 🟡 Gemini spot check 2026-09-09 (`runs/eval_concept_image/20260909_v3_spotcheck_gemini`, `…_v3b_…`, `--ids`): Power Ranger beak back; the Nittany Lion under one combined beak sentence got a duck bill on the mascot face → split into two sentences chosen per subject → full mascot head, striped scarf, no beak. GPT Image rerun + judge-v2 gate still pending OpenAI credits (probe 14:33: `credit_balance_exhausted`) |


### 68c — GPT Image only + judge-gated turn 2 (2026-09-10, operator: "gemini seems like a bad model to use", "we might not have to always default to turn 2… why would we assume so?", "go")

|  | Happy | Provider/turn failure | Prompt contract drift | Wrong inputs reach the model | Regression |
|---|---|---|---|---|---|
| Recipe provider policy (`concept_recipe.provider_order`, v4 `provider_order: ["openai"]`) | `creative_agent/tools/tests/test_concept_recipe.py::test_provider_order_is_openai_only_and_refuses_gemini` | `::test_operator_recipe_fails_closed_when_openai_unavailable` (no key → ok=False, `error_kind=provider_unavailable`, zero provider calls, no file); `::test_operator_recipe_classifies_quota_and_content_failures` (429/credits → provider_unavailable; content policy → generation_failed) | `::test_recipe_json_is_versioned_and_complete` (v4, openai only, `turn2_policy=when_needed`, `max_turn1_rolls≥2`) | `::test_operator_recipe_refuses_gemini_instead_of_using_it` (`provider_not_enabled`, no calls) | `::test_operator_recipe_never_attaches_reference_images` |
| Turn-1-only tool + targeted edit tool (`media._generate_with_operator_recipe(turn2=)`, `media.edit_duck_concept_image`) | `::test_operator_recipe_turn1_only_by_default_and_leaves_turn2_to_the_gate` (turn2_status=skipped, reason=gated_by_caller); `::test_edit_duck_concept_image_is_openai_only_and_fails_closed` | `::test_operator_recipe_keeps_turn1_when_edit_fails` (turn2=always) | `::test_operator_recipe_runs_turn1_then_edit_turn_when_told_always` (edit → `_turn2.png`, copied to output) | edit tool: empty prompt / missing input → generation_failed; registry `root` kwarg tolerated | `tool_registry` table + `duck_tools.__all__` carry `edit_duck_concept_image` |
| Gate decision (`duck_creative_agent/concept_image_gate.py::plan_turn2`) | `creative_agent/runtime/tests/test_concept_image_gate.py::test_plan_clean_pass_needs_no_edit` | `::test_plan_without_structured_verdict_edits_nothing` (judge unavailable/error/dry_run → nothing edited) | `::test_plan_named_logo_edits_and_names_the_mark` ("Remove the Penn State logo. Change nothing else."), `::test_plan_thin_part_edits_and_names_the_part`, `::test_plan_logo_and_thin_part_become_one_edit` | `::test_plan_judgment_calls_are_left_for_review_not_edited` (unnamed logo / missing beak / missing trait → no edit); `::test_plan_body_drift_rerolls_instead_of_editing` | `::test_plan_policy_never_and_always` |
| Gate orchestration (`run_gated_concept_image`) | `::test_run_clean_image_generates_once_judges_once_and_ships_turn1`; `::test_run_named_logo_edits_then_rejudges_and_ships_the_edit` | `::test_run_edit_failure_keeps_turn1_and_flags_it`; `::test_run_generation_failure_propagates_error_kind`; `::test_run_judge_unavailable_edits_nothing` | `::test_run_edit_that_loses_a_keep_trait_is_reverted`, `::test_run_edit_that_breaks_the_body_is_reverted` | `::test_run_body_drift_rerolls_up_to_the_recipe_limit_then_stops`, `::test_run_body_drift_then_clean_reroll_ships_the_second_roll` | `::test_run_policy_always_delegates_turn2_to_the_tool`, `::test_run_policy_never_judges_turn1_only`; receipts `::test_gated_generated_visual_merges_tool_result_with_the_gate`, `::test_gated_generated_visual_keeps_failure_and_error_kind` |
| One judge contract (`creative_policies.semantic_review_contract` / `judge_concept_image` / `build_visual_policy_qa(semantic_review=)`) | `test_creative_policies.py::test_judge_concept_image_uses_the_same_contract_as_policy_qa` | n/a | `::test_judge_prompt_is_the_form_and_its_definitions_not_the_old_essay` (459 → 385 words; essay paragraphs about uploaded references / protected copying gone; "name every logo…" present) | `::test_build_visual_policy_qa_reuses_a_precomputed_semantic_review` (the judge must not run twice for one image) | existing judge v1/v2 tests unchanged |
| Production wiring (`tasks._run_concept_bundle`, `viewer_data.regenerate_run_visual`, Studio `process_studio_concept_image`) | `test_viewer.py::test_process_studio_concept_image_text_only_runs_operator_recipe_with_gated_turn2` (recipe kwargs, no refs, edit named, judged turn 1 then turn 2, receipt `concept_image_gate`) | ✅ live fail-closed check 2026-09-10 with OpenAI still out of credits: `eval_concept_image.py --mode treatment --ids power_ranger` → `error_kind=provider_unavailable` in 2.4 s, no image, no Gemini | legacy Studio path kept behind `operator_recipe: false` (`::test_process_studio_concept_image_supports_text_only_generation`, `…_prioritizes_theme_relevant_style_memory`, `…_retries_failed_semantic_visual_qa`) | `test_runner.py` + `test_concept_image_request.py` (request builder unchanged) | 🟡 no unit test drives `_run_concept_bundle` through the gate with a fake registry; add with the first live GPT Image run |
| Legacy concept path + character sheets (`media._provider_order`, `_character_sheet_provider_order`) | `test_tools.py::test_media_tool_openai_fallback` (now the only path) | `::test_generate_duck_character_sheet_fails_closed_instead_of_falling_back_to_gemini` | `::test_generate_duck_concept_image_auto_is_openai_only_and_refuses_gemini` (auto → openai only; gemini → provider_not_enabled; no key → provider_unavailable) | source-anchored legacy requests now use `openai_edit_image` (was gemini_edit) | `::test_generate_duck_character_sheet_auto_uses_single_full_sheet_source_edit`, `…_falls_back_to_single_view_and_crops_contact_sheet_views` unchanged; social remix (`::test_media_tool_source_remix_uses_gemini_edit`) deliberately still Gemini |
| Thursday lane (`flows/thursday/steps.py`) | `tests/test_thursday_batch_policy.py::test_generate_duck_images_uses_style_lab_primary_generator` (GPT Image `gpt-image-2`, no reference photos, debug `image_model` + `reference_photos_attached=false`) | missing `OPENAI_API_KEY` raises → option skipped by `step_thursday_batch_prepare` (existing guard); no DALL·E / Gemini fallback | `::test_generate_duck_images_retries_when_semantic_qa_fails` (contract-gate retries on GPT Image; Gemini kept for analysis) | style memory shapes the contract as text only | 🟡 Thursday still uses its own contract prompt, not the operator recipe; migrate after the first reviewed GPT Image Thursday run |
| Studio Image Tools edits (`studio_image.process_studio_ai_image_edit`) | `test_viewer.py::test_process_studio_duck_edits_run_on_gpt_image_with_the_operator_turns` (`simplify_for_print` = recipe turn-2 text, `remove_logo`) | n/a | `::test_process_studio_ai_image_edit_records_edited_asset` (background scenes stay Gemini; `provider_note` None) | stale `provider: gemini` on a duck edit → OpenAI + `provider_note` | UI: `viewer.py` action lists + `imageEditProvider` options; `node --check` on the inline script passed 2026-09-10 |
| Concept form (`action_registry`) | `test_action_registry.py` (updated `visible_when`) | n/a | provider options Auto (OpenAI GPT Image) / OpenAI only; `gemini_image_model` field legacy-only | n/a | n/a |
| Eval harness (`scripts/eval_concept_image.py`) | `--turn2 when_needed\|always\|never`, `--provider auto\|openai`; records carry `turn2_reason`, `turn1_qa_status`, `rolls`; summary `turn2_statuses`, `provider_failures` | dry run exercised 2026-09-10 | — | — | ✅ 2026-09-11 after credits returned (`runs/eval_concept_image/20260911_*`): **gated 19/20 pass, 1 needs_review (Headless Horseman: the judge reported keep-trait "no head" missing and it is right, the duck came out with a head; the eye had glossed over it), 0 body drift, 1 edit in 20 → gate PASS, 19/20 acceptable by eye; operator: "those are perfect"** (first 8 ingested as `operator_pick` style memory). Always-edit on the six operator ducks: 5/6 pass but the unneeded edit flattened the Nittany Lion's face, muddied the Horseman, and got Jason flagged → worse on 3 of 6. Bare words (`--mode bare`, the operator's six words, nothing added): 2/5 pass, 3 fail on logos/text (dinosaur buckle, PENN STATE jersey, Sleepy Hollow sign), 1 body drift (horseman on a horse), 1 blocked by OpenAI's safety system (Abu), scene backgrounds on every image → the added sentences stand in for ChatGPT's memory and earn their place. **Recipe v7 (ChatGPT's constraint block + the operator's render look) PROMOTED 2026-09-13 (operator: "I like v7"):** six operator ducks 6/6 on the constraint block (v6), the two hardest cases 2/2 on v7, and the 14 non-operator cases as a background check: 13/13 judged pass, 0 drift, 0 edits needed; the 14th (`visible_silhouette`, "Superhero as a rubber duck") was `moderation_blocked` by OpenAI's output safety filter (`generation_failed`, fail-closed with the reason; it had passed on v4, so the block is nondeterministic). Follow-up: one automatic re-roll on `moderation_blocked` before reporting the failure |


## Surface 69 — Concept-to-print pilot: image → 3D AI Studio → paint-to-print handoff (DIAGNOSIS 2026-09-13, operator: "Go")

|  | Happy | Provider/turn failure | Prompt contract drift | Wrong inputs reach the model | Regression |
|---|---|---|---|---|---|
| Pilot script (`duckAgent/scripts/print_bundle_pilot.py`, manual, spends a 3D AI Studio credit per `--image` run and uploads the image to the Shopify CDN) | ✅ 2026-09-13 live: v7 Highland Cow image → tripo p1 (124 s) → handoff READY (8 colors, 76 components, 9 tiny islands, all 7 required gates pass) in `runs/studio_assets/3d_lab/print_bundle_pilot_20260913_235343_highland-cow-duck` | submission without a task_id or a terminal provider status raises with the failure reason; polling capped by `--max-minutes` | n/a | `--glb` mode skips generation (no credit) | ✅ 2026-09-13: May's Highland Cow GLB reconverted with today's converter = May's manifest exactly (6 colors, 122 components, 7 tiny islands, ready, 12 s) → paint-to-print-3d has not rotted |
| Mesh engine test 2026-09-15 (`threedai_bridge` gains `generate_parts` for Tripo 3.1, texture-free by API rule; `test_threedai_bridge.py::test_submit_threedai_single_image_tripo31_can_request_parts_and_p1_never_does`) | ✅ live: Tripo 3.1 print_detail → 469k faces (267 s), handoff READY but still 8 browns; Tripo 3.1 parts → **11 separate meshes** (body, head, hair, beak, 2 eyes, 2 horns, 2 wings, ear) in 164 s, rendered per part in `print_bundle_pilot_20260915_tripo31_parts/parts_preview.png` | first parts request rejected 400 "generate_parts is not compatible with texture=true or pbr=true" before any credit; fixed in `71cc76b` | Tripo P1 caps `face_limit` at 20k (docs), which explains every ~5k-face duck since May | paint-to-print's lane cannot ingest a texture-less GLB (needs TEXCOORD_0 + material) | 🔴 NEXT: the parts route (label parts → one concept-palette color per part → per-part repair → multi-object 3MF at 57 mm with AMS slots → 3-angle render), replacing texture quantization |
| What the gates do not measure (the real gaps, see roadmap Priority 3 slice 1) | — | — | 🔴 OPEN: colored preview is mottled brown, not the concept's 4–5 flat regions (gate `visual_color_confidence` passed anyway); no region-cleanliness gate exists | 🔴 OPEN: 3MF height ≈108 "mm" (voxel units), no `target_height_mm`; no AMS slot mapping | 🔴 OPEN: `concept_to_print_status.json` never sees 3D Lab / pilot runs (`viewer.py` task filter) |


## Surface 70 — Meme Monday "replace": shortlist in the email, `meme replace N` by reply (2026-09-14, operator: "I don't like the duck we chose… the email can send 5 other duck options and I can reply to replace")

|  | Happy | Provider/turn failure | Prompt contract drift | Wrong inputs reach the model | Regression |
|---|---|---|---|---|---|
| Shortlist (`helpers/meme_helper.py::score_meme_products` extracted from the chooser; `meme_product_shortlist`) | `tests/test_meme_replace.py::test_shortlist_is_the_runners_up_without_the_chosen_duck` (excludes the pick and empty handles, numbers 1–5, image url) | shortlist failure never blocks the meme (`step_meme_prepare` try/except → `[]`) | `::test_prepare_records_alternates_and_honors_a_product_override` (`meme_alternates` in state; `product=` override skips the chooser) | n/a | chooser behaviour unchanged: existing `test_meme_repeat_avoidance.py`, `test_meme_recently_used_filter.py`, `test_meme_occasion_bias.py` pass |
| Monday email (`step_meme_email` "Swap the duck" section) | `::test_email_lists_the_swap_shortlist_with_the_reply_instruction` (numbered titles + product links, "meme replace N", deadline = slot − 30 min) | deadline text falls back when the slot cannot be computed | 🟡 manual: the first live email with the section goes out Mon 2026-09-21 09:00; read it as delivered | n/a | section absent when there are no alternates |
| Reply command (`main_agent.infer_reply_action` → "replace"; `flows/meme/replace.py::parse_replace_choice`) | `::test_parse_replace_choice_reads_numbers_and_handles`; `::test_handle_mail_event_routes_meme_replace` (body "meme replace 2" → `replace_meme_duck(choice=2)` + feedback email) | `::test_handle_mail_event_replace_refusal_emails_the_reason` (ReplaceRefused → "failed" feedback email with the reason) | "replace" is checked before publish/apply/approve and is NOT in `DUCK_OPS_DECISION_GATEWAY_ACTIONS` (`::test_infer_reply_action_recognizes_replace_and_the_gateway_leaves_it_alone`) | quoted original text is ignored (reply-visible block) | existing `test_email_decision_gateway.py` pass |
| The replace (`flows/meme/replace.py::replace_meme_duck`) | `::test_replace_cancels_regenerates_and_never_publishes` (scheduled FB post found on the page's list and deleted, IG entry quarantined, `meme_replacements` recorded, publish keys reset, prepare/render/resolve/email in order with the product override, NO publish call) ; `::test_replace_accepts_a_handle_when_there_is_no_shortlist`; `::test_replace_before_any_schedule_skips_facebook_and_instagram` | `::test_replace_refuses_when_the_facebook_post_is_no_longer_scheduled` (already live / already deleted → refuse, nothing touched); `::test_graph_client_refuses_without_credentials` | `::test_replace_is_refused_inside_the_grace_window` (posts at 18:00, replacements close 17:30, state untouched) | `::test_replace_refusals` (already posted; number outside the shortlist; same duck) | ✅ live 2026-09-14: CLI `scripts/meme_replace.py --handle bigfoot-duck` deleted the scheduled Alabama post, regenerated, emailed, and the watcher scheduled Bigfoot for 18:00 on FB + IG (page's only scheduled post). The first version forced a second publish and RACED the always-on mail watcher (`flows/ops/watch_publish_idle.py`), which wrote "blocked" over a good schedule → the module never publishes; the CLI waits for the watcher and then `--reconcile`s state against the live schedule (used to repair 2026-09-14) |
| Auto-mode permission classifier | — | 🟡 Claude's session cannot run Tier-3 Meta mutations (scheduled-post delete, regeneration writes) as ad-hoc scripts; the repo's own `scripts/meme_replace.py` ran. Keep operator actions as repo scripts, not heredocs | — | — | — |


## Surface 71 — Parts route: part-aware 3D model → one filament per part → Bambu multi-part 3MF (SPEC-FIRST 2026-09-15, operator: "So the colors here look pretty bad… did u do this just as a sample?" → "Can u do the power ranger first?")

Replaces texture quantization for print color: Tripo 3.1 `generate_parts` returns ~9–11 semantic parts; a vision labeler names each part and its concept color; `paint-to-print-3d/src/color3dconverter/parts_bundle.py` (offline, no keys) thickens every part into a closed shell, adds a voxel core, scales to 57 mm Z-up and writes ONE assembly object with per-part Bambu extruders. First articles: Power Ranger (9 parts, 4 slots) and Highland Cow (11 parts, 4 slots), both `ready` on 2026-09-15.

|  | Happy | Provider/turn failure | Prompt contract drift | Wrong inputs reach the model | Regression |
|---|---|---|---|---|---|
| Load + facts (`parts_bundle.load_scene_parts`, `part_facts`, `detect_mirror_pairs`) | `paint-to-print-3d/tests/test_parts_bundle.py::test_load_scene_parts_keeps_scene_order_and_refuses_single_mesh` (numeric-suffix order, welded seams), `::test_part_facts_find_mirror_twins_and_positions` | single-mesh GLB → `ValueError("multi-part")`, never a silent one-part bundle | facts text (`description`) is what the labeler reads; twin/position words asserted | GLB scene order is NOT stable across export (trimesh reversed a 5-part scene) → parts sort by name suffix | Tripo parts arrive as 20–90 unwelded patches per part: `merge_vertices(merge_tex, merge_norm)` → one body each (verified on both real models) |
| Concept palette (`extract_concept_palette`, `snap_color`, `build_materials`) | `::test_extract_concept_palette_drops_the_backdrop`, `::test_build_materials_snaps_merges_and_orders_by_surface` (slot 1 = largest surface; twins share a slot; `max_slots` squeeze merges the closest pair and records it) | no foreground → `ValueError`; unsnappable color keeps the labeler's hex (ΔE > 15) | — | shading splits one material into 2–3 pixel clusters, so clusters are evidence only; the labeler names the material | cream horns snapped onto tan at ΔE 25 → threshold 15 (2026-09-15) |
| Vision labeler (`duckAgent/…/parts_labeler.py::label_parts_with_vision`, `validate_label_map`, `refine_part_with_vision`, `resolve_label_map`) | `creative_agent/runtime/tests/test_parts_labeler.py::test_resolve_label_map_vision_path_validates_and_writes_the_map`, `::test_chat_payload_adapts_to_reasoning_models` (gpt-5 family: no temperature, `max_completion_tokens`, `reasoning_effort`) | `::test_no_api_key_and_no_override_is_unavailable_not_a_guess`, `::test_vision_failure_is_recorded_not_raised` → `source: unavailable`, bundle gate fails visibly (no heuristic map) | `::test_validate_label_map_applies_size_position_and_twin_priors` (eye ≤ 6 % of surface, tail at the back, beak at the front, wing off-center, twins share a color, largest part is never a detail), `::test_validate_label_map_reports_coverage_and_color_format` | `::test_resolve_label_map_refines_a_flagged_part_once` (targeted second look with the objection), `::test_resolve_label_map_stays_needs_review_when_refine_does_not_fix_it`, `::test_override_file_wins_and_skips_vision` | ✅ real: gpt-4o mislabeled a hidden wing "eye/black" and the diamond band "tail/red" from a fixed front view; best-view tiles + geometry facts + gpt-5.5 → 9/9 (Ranger) and 11/11 (Cow) correct, validator clean (2026-09-15) |
| Repair (`solidify_part`, `build_core`, `repair_part` ladder shell → recessed fill → fill → MeshFix) | `::test_solidify_part_makes_a_closed_shell_and_keeps_the_outer_surface` (outer surface untouched, inside-out winding still oriented by geometry), `::test_build_core_lies_strictly_inside_the_figure`, `::test_repair_parts_closes_open_shells_without_changing_extents`, `::test_boundary_loops_and_recess_push_caps_inside` | a rung that eats or balloons a part (> 3 % beyond what the method legitimately moves) is refused (`::test_repair_part_refuses_a_fix_that_eats_the_part`); `method: failed` fails the required gate | — | flat caps across open loops coincide with the neighbour's cap and cut through concave dishes (Ranger visor mottled white/black in the QA render) → shells, no membranes; core fills the interior so the print is not hollow | 🟡 manual: open `parts_bundle.3mf` in Bambu Studio and confirm "1 object, N parts" with the slot map; the BambuStudio 02.05 CLI segfaults in `PartPlateList::set_shapes` on any input, so no headless slice check exists |
| Assembly + 3MF (`assemble_parts`, `place_parts`, `write_parts_3mf`, `read_parts_3mf_structure`, `write_parts_colorgroup_3mf`) | `::test_assemble_parts_scales_to_height_on_the_bed_z_up` (Y-up → Z-up, 57 mm, bottom on z=0), `::test_write_parts_3mf_has_one_assembly_with_parts_and_bambu_extruders` (mesh objects, components, one build item, `Metadata/model_settings.config` part ids = mesh ids, extruders 1..slots, no `BambuStudio` application tag) | structure gate reads the file back (`threemf_structure_valid`) | — | — | colorgroup fallback file kept for the painted-import path |
| Bundle + manifest (`build_parts_bundle`; bridge `build_parts_bundle_with_color3dconverter`; `scripts/parts_bundle_pilot.py`) | `::test_build_parts_bundle_ready_with_a_valid_label_map` (all required gates, `handoff_manifest.json` with the texture route's schema + `route: parts_bundle`, QA board + 3 views, markdown) | `::test_build_parts_bundle_needs_review_when_labels_are_heuristic_or_broken` (heuristic/unavailable labels → `review_required`, file still written for a manual look; broken map → no 3MF) | advisory gates: `filament_slots_within_ams` (4), `thin_parts_printable` (≥ 1.2 mm), `core_filled` | operator override `parts_labels.json` → `--no-vision` rebuild (used to rebuild both articles without new calls) | ✅ real 2026-09-15: Ranger 9 shells + core, 4 slots (red / white collar+diamonds+trim / yellow / black), 12 MB; Cow 11 shells + core, 4 slots; both `ready` |
| Blast radius of the hole-repair fix | ✅ swept 40 recent meme + jeepfact subject photos: 36 unchanged, 4 changed (3 jeepfact, 1 meme), and all 3 jeepfact cutouts eyeballed on magenta — clean, no false holes; the changed pixels were real openings (gap under a hat brim, beside a raised hand, beside a beak) | — | — | — | 🟡 **newduck NOT swept**: `flows/newduck/steps.py:384` sets `preserve_internal_holes` from `NEWDUCK_LISTING_IMAGE_PRESERVE_INTERNAL_HOLES` (default true), so the listing-image path takes the same fix, but no newduck subject photos exist under `runs/` to sweep. Check the first newduck listing images rendered after 2026-09-22 |

| Known gap — route selection | 🔴 MISSING | — | — | — | **no test picks parts vs painted per duck.** Parts are correct only when a color boundary is also a SHAPE boundary (cow: horns/hair/beak). When the color is paint on a smooth surface the generator invents bad relief and parts inherit it (Ranger 2026-09-16: diamond band split off on ONE side, flat wings, deep helmet groove; operator: "the power ranger looks pretty bad"). The signal is already in the label map (`pattern`/`trim`/`collar` labels, or a row with an `also` color) — add the chooser + a test on both first articles |
| Known gap — mottle gate | 🔴 MISSING | — | — | — | quantizing a baked texture into N flat colors gives camouflage at ANY N, because the generator bakes lighting in and k-means splits by brightness before material. Re-confirmed free 2026-09-16 (cow textured GLB, `n_regions=32`: horns, eyes and beak gone). A region-cleanliness gate that FAILS this output is still unbuilt (planned since the 2026-09-13 diagnosis); the fix is snapping to the concept's named material colors, never discovering colors from the texture |
| Known gap — surface graphics | — | — | — | — | graphics the generator did NOT split into parts print in the part color (the Ranger's eye slits are relief on the helmet); next layer: per-part texture projection or a `pattern` split |
| Known gap — untried engines | 🟡 manual | — | — | — | `submit_threedai_multiview` exists in the bridge with ZERO callers and has never run; Hunyuan Pro (8 view slots, 1.5M faces) and TRELLIS.2 have never been called. Every duck so far is Tripo. Wire and run before claiming an engine comparison |

## Surface 72 — Cutout hole repair fills real openings with the studio backdrop (2026-09-21, operator: "This is a meme from today but the background removal was poor. Are we just using the cheapest local model?")

The Monday meme posted with the golf cart's windshield filled solid white. **Not the model:** the cutout is local rembg/u2net and all five cached models (u2net, u2netp-class silueta, isnet-general-use, birefnet-general-lite, bria-rmbg) cut that windshield correctly when tested on the same photo. The defect was `preserve_internal_holes: True` in `helpers/social_template_renderer.py::_clean_subject_for_template`, which runs `studio_image._restore_internal_alpha_holes`: it refills any enclosed transparent region from the SOURCE pixels, and for a real opening those source pixels are the white studio backdrop. Fix: the repair now skips a hole whose source colour matches the photo's backdrop (border-median RGB, tolerance 60), so false holes through the subject are still repaired and genuine openings stay open.

|  | Happy | Provider/turn failure | Prompt contract drift | Wrong inputs reach the model | Regression |
|---|---|---|---|---|---|
| Hole repair decision (`studio_image._restore_internal_alpha_holes`, `_border_median_rgb`, `_component_matches_backdrop`) | `creative_agent/runtime/tests/test_viewer.py::test_process_studio_clean_photo_repairs_a_false_hole_through_the_subject` (dark subject-coloured hole → refilled, `skipped_backdrop_components == 0`) | repair never raises; on no source image the guard is inert and old behaviour stands | guard is defeatable: `::test_process_studio_clean_photo_can_force_repair_of_a_backdrop_opening` (`internal_hole_skip_backdrop_matches: False`) | area cap (`internal_hole_max_area_ratio`) unchanged and still applies first | ✅ `::test_process_studio_clean_photo_leaves_a_real_opening_transparent` pins the golf-cart case: backdrop-coloured opening stays transparent, `skipped_backdrop_components == 1` |
| Cutout model choice | real-photo comparison 2026-09-21: 5 rembg models on the failing photo, all correct; u2net 1.6 s vs bria-rmbg 11.8 s → no reason to change the default | fallbacks in `meme_helper._prepare_meme_product_image_bytes` return the ORIGINAL png on any rembg error (white background ships) — 🟡 no alarm on that path | — | — | all six models cached in `~/.u2net`; `SOCIAL_TEMPLATE_REMBG_MODEL` / `MEME_PRODUCT_CUTOUT_MODEL` override |
| Regression sweep | 6 recent product photos (golf cart, alabama, monster truck, bigfoot, lifeguard, boxer) before/after: only the golf cart changed, 4 holes left open (290,662 px); the other five had zero holes either way | — | — | — | 🟡 manual: the repair pass had NEVER fired on any other recent photo, so its protective value is unproven; revisit if a false hole appears |
...
## Surface 74 — Operator override queue for This or That Thursday (2026-09-24, operator: "i would like to add some override section to this or that thursday, where i can just add what i want to be this or that, if it exists then i want it to override options that the agent comes up with (that has been bad as of late)")

**The lane this bypasses.** Thursday builds three head-to-head options from a tiered candidate funnel (`build_next` → `etsy_trending` → `competitor_trending` → `top_performer_7d` → static pool), then an LLM qualifier decides which pairings are publishable. The operator's verdict on that qualifier in 2026-09 was that it "has been bad as of late", and the lane already skips ~50% of weeks (`[[project_thursday_robustness_followups]]`). An override lets them name the matchup outright, and doubles as a way to run the week when the funnel comes up empty.

**Store:** `duckAgent/config/thursday_overrides.json` (`overrides_version: 1`), read/written by `helpers/thursday_overrides.py`. An entry is a full matchup — `duck_a` + `duck_b`, optional `note` and `phrase_a`/`phrase_b`. **The operator's words are used verbatim as the image phrase** unless they supply an explicit phrase, so nothing re-interprets the idea between the config and the prompt (that re-interpretation is what they are overriding). List order is priority order; the first three fill Thursday's three slots and the remainder wait.

**Gate policy, chosen explicitly by the operator: skip the qualifier, keep image QA.** `_review_thursday_batch_option` is not consulted for an override — that judgment is the thing being overridden. Style Lab image QA is untouched, so a bad render is still pulled before the email, and the rejected option carries its `override_id` so the operator is told *which* of their ideas failed. A pulled override is **not** retired; it was never accepted and returns next week.

**Retirement:** publishing an override's option moves it from `matchups` to `accepted` with run id + option id (operator: "if i end up accepting the this or that from the override, we can remove it from the override list"). `override_id_for_option` requires BOTH sides to share the entry, so a half-override pairing can never consume it. Retirement runs *after* the post is live and can never fail the publish (`[[feedback_receipts_never_load_bearing]]`) — worst case the entry stays queued.

**Isolation (architectural convention #4 — this is a WRITTEN production config, so all three layers):** (1) autouse `_isolate_thursday_overrides` in `duckAgent/tests/conftest.py` redirects `CONFIG_PATH` to tmp; (2) source guard raises `TestModeRefusalError` when `DUCK_TEST_MODE=1` and the resolved write path is the frozen factory default; (3) post-suite audit `TestShippedConfigIsSane::test_no_test_fixtures_leaked_into_the_real_queue` scans the live config for fixture markers. Only duckAgent writes this file, so duck-ops needs no redirect.

**Visibility (operator asked for both surfaces):** the Thursday batch review email marks override options (`Your Override` eyebrow / `[YOUR OVERRIDE]` in plain text), lists what is still queued behind the three slots, and names entries it could not parse; `/portal/intel/thursday-overrides` shows next-up / waiting / unreadable / accepted, and is listed in the intel directory.

|  | Happy | Bad operator input | Funnel empty | Image QA fails | Replay / idempotency | Regression |
|---|---|---|---|---|---|---|
| Store (`helpers/thursday_overrides.py`) | `tests/test_thursday_overrides.py::TestCandidateConversion` (verbatim words, explicit phrase wins, outscores the funnel) | `::TestValidation` (missing side, duck vs itself, typo reported not silently dropped) | `::TestLoading::test_missing_file_means_no_overrides_not_a_crash`, `::test_corrupt_file_degrades_instead_of_taking_thursday_down` | n/a | `::TestRetirement::test_replaying_a_publish_cannot_corrupt_the_list` | `::TestSlotLimit` (3 used, rest reported waiting — `[[feedback_no_silent_caps]]`) |
| Funnel injection (`flows/thursday/steps.py::step_thursday_batch_prepare`) | `tests/test_thursday_override_wiring.py::TestFunnelInjection::test_override_takes_option_one_ahead_of_the_funnel`, `::test_three_overrides_fill_every_slot` | `::test_an_unreadable_override_config_does_not_stop_thursday` | ✅ `::test_overrides_carry_the_flow_when_the_funnel_is_empty` (an override alone can run the week) | n/a | n/a | `::test_operator_phrase_reaches_image_generation_verbatim` |
| Qualifier bypass | `::test_the_qualifier_is_not_consulted_for_an_override` (**the core request**) | n/a | n/a | `::TestImageQaStillApplies::test_an_override_whose_art_fails_qa_is_pulled_and_named` | n/a | `::test_the_qualifier_still_runs_on_generated_options` (bypass must not leak to generated options) |
| Retirement on accept (`::_retire_thursday_override_if_accepted`) | `::TestRetirementOnAccept::test_publishing_an_override_removes_it_from_the_list` | `::test_no_selected_option_is_a_clean_skip` | n/a | `::TestImageQaStillApplies::test_a_pulled_override_is_not_retired` | `::test_retirement_failure_never_looks_like_a_failed_publish` | `::test_publishing_a_generated_option_retires_nothing` |
| Review email (`_build_thursday_batch_review_email_content`) | `::TestReviewEmail::test_override_options_are_marked_in_both_formats` | `::test_a_malformed_entry_is_reported_not_hidden` | `::test_no_queue_section_when_there_is_nothing_to_say` | n/a | n/a | `::test_the_waiting_queue_is_listed_so_nothing_is_silently_capped` |
| Portal page (`/portal/intel/thursday-overrides`) | `creative_agent/runtime/tests/test_thursday_overrides_page.py::TestRender` (next-up/waiting/accepted, notes + phrases, escaping) | `::test_invalid_entries_are_named_not_hidden` | `::test_empty_queue_says_the_funnel_runs_normally` | n/a | n/a | `::TestRegistration` (route in viewer + listed in intel directory — `[[feedback_loader_and_registration_both_need_tests]]`) + `::TestAgreesWithTheFlow::test_page_split_matches_the_helper_the_flow_uses` |

**Bug caught by the existing suite while wiring this:** `selected_opt` in `step_thursday_publish` is only bound inside the batch branch, so adding a call that referenced it after the branch raised `cannot access local variable 'selected_opt'` on the single-post fallback path. `tests/test_social_post_receipts.py::test_thursday_publish_saves_instagram_receipt` caught it; fixed by initialising `selected_opt = None`.

**Note on node IDs:** the 3-root pytest command reports failures under `creative_agent/runtime/<file>` even for tests that live in `tests/` — `creative_agent/runtime/pyproject.toml` moves pytest's rootdir, so printed paths are not real. Find the file with `find . -name <file>` rather than trusting the node ID.

## Surface 75 — Model registry + Etsy tag policy enforcement (2026-09-25, operator: "what models are we using to develop the listing now? Should we start upgrading?" then "Eval all workflow models while we are at it")

**What the audit found.** The model was an inline string literal at 25+ call sites. All four newduck jobs — reading the product photos, the staged visual brief, the listing copy, the copy hardener — ran on `gpt-4o-mini`. `gpt-3.5-turbo` (2023) was drafting **customer refund and replacement offers** in `flows/reviews/etsy_review_helper.py`; `review_selection_helper.py` was on plain `gpt-4`. Meanwhile the single `gpt-5.5` reference in the repo was a 3D part-naming utility. The customer-facing copy had the weakest models in the system and an internal tool had the strongest.

**Why nothing had ever been upgraded: a model swap was not actually possible.** The gpt-5 family takes `max_completion_tokens` + `reasoning_effort` and **400s on `temperature`**; classic models are the reverse. Every call site hand-built a `temperature`/`max_tokens` payload, so pointing any of them at gpt-5 was an immediate API error. `helpers/openai_helper.py` had no gpt-5 awareness at all.

**Fix: `config/models.json` (roles) + `helpers/model_registry.py` (resolution + payload normalization).** Roles are named by JOB, not flow, so the same job stays consistent across lanes. `DUCK_MODEL_<ROLE>` overrides any role at runtime, making a rollback or an A/B a shell variable instead of a commit. `normalize_chat_payload` is called inside `openai_chat` — the one chokepoint that `openai_json` and `openai_json_with_debug` both route through — so every existing call site works against either family unchanged. `temperature: 0` maps to `reasoning_effort: low` rather than being silently dropped, preserving the determinism contract that cached classifiers and gated evals depend on (LLM-output surface recipe, temp-0 rule).

**A/B on the live Headless Horseman photos** (same four staged images, `_analyze_newduck_staged_images`): gpt-4o-mini returned 4 visible features ("headless design", "black cape", "carved pumpkin", "smooth finish"); gpt-5.5 returned 8, correctly read the pumpkin as a **head accessory** replacing the duck head (the entire product concept), and caught the cravat, high collar, belt buckle, visible tail and print layer lines. It also produced a broader `avoid_terms` set (licensed character, brand name, team mascot, official logo), which is the IP guard. Note the vision call sends images at `"detail": "low"` (512px downsample) — an open lever, untouched here.

**Etsy tag policy — three gates each enforced a TITLE rule on TAGS.** The Headless Horseman listing shipped tagged `christmas duck`. Not an LLM slip: `_build_etsy_listing_tags` padded every listing to 13 from a hard-coded fallback list with `"christmas duck"` third in it, so any duck needing padding got tagged for Christmas in any season. The same builder had no Jeep tag at all, so the operator's one-Jeep-tag policy ([[feedback_etsy_title_tag_policy]]) was **never implemented** — and could not be, because three independent gates blocked it: `_normalize_etsy_tag`'s blocklist, `_validate_etsy_tags`, and newduck's `unsupported_material_claim` scan (`\brubber ducks?\b`). Fixing one would have looked correct in isolation and still shipped no Jeep tag — [[feedback_incomplete_fix_enumerate_all_branches]] in its exact form. All three now exempt the one allowed policy tag (`jeep rubber duck`, or `custom jeep duck` on a customizable listing), and `_validate_etsy_tags` flags **two or more** as the violation, which is what the policy actually targets.

Seasonal tags are now derived from the duck (`_seasonal_tag_for`), never invented by padding, and an AI-suggested holiday that is not this duck's holiday is dropped (`_is_wrong_season_tag` catches the whole family — "christmas gift", "santa duck" — not just the exact fallback string).

|  | Happy | Wrong input | Model-family drift | Policy violation | Regression |
|---|---|---|---|---|---|
| Role resolution (`helpers/model_registry.py`) | `tests/test_model_registry.py::TestResolution::test_role_resolves_to_its_configured_model` | `::test_unknown_role_uses_the_callers_default`, `::test_a_broken_config_does_not_take_every_flow_down` | `::TestReasoningFamilyDetection` (gpt-5/o1/o3/o4 vs classic) | n/a | `::test_env_override_wins_so_a_rollback_needs_no_commit` |
| Payload normalization | `::TestPayloadNormalization::test_temperature_is_dropped_for_a_reasoning_model`, `::test_max_tokens_becomes_max_completion_tokens` | `::test_a_payload_without_a_model_is_returned_unchanged`, `::test_non_dict_input_does_not_raise` | ✅ `::test_reasoning_effort_is_stripped_when_rolling_back_to_a_classic_model` (rollback direction, or DUCK_MODEL_* is one-way) | n/a | `::test_the_ceiling_is_raised_so_reasoning_does_not_eat_the_answer` (reasoning tokens bill against the same limit → empty content at finish_reason=length), `::test_temperature_zero_becomes_low_reasoning_effort` |
| Wired at the chokepoint | `::TestNormalizationIsWiredIntoEveryCall::test_openai_chat_normalizes_before_posting` | n/a | n/a | n/a | `::test_both_json_helpers_reach_openai_chat` (a helper that bypasses openai_chat silently misses normalization) |
| Registry contents | `::TestShippedConfig::test_the_real_config_parses_and_every_role_names_a_model` | n/a | n/a | ✅ `::test_customer_facing_roles_are_not_on_the_cheapest_model` | `::test_describe_roles_reports_the_effective_model` |
| Seasonal tags (`_build_etsy_listing_tags`) | `tests/test_etsy_tag_policy.py::TestNoInventedSeason::test_season_is_derived_from_the_duck` (8 parametrized) | `::test_a_plain_duck_gets_no_holiday_tag_at_all` (padding must not guess) | n/a | ✅ `::test_a_halloween_duck_is_not_tagged_for_christmas` (**the live bug**) | `::test_a_christmas_duck_still_gets_christmas`, `::test_seasonal_detection_reads_the_title_too` |
| One-Jeep-tag policy | `::TestJeepTagPolicy::test_every_listing_gets_exactly_one_jeep_tag` | `::test_a_customizable_listing_uses_the_custom_jeep_tag` | n/a | ✅ `::test_never_two_jeep_tags_even_when_the_ai_suggests_one` | `::test_the_jeep_tag_survives_the_thirteen_tag_cap` (required tags sit ahead of padding) |
| Still a valid Etsy tag set | `::TestStillAValidEtsyTagSet::test_a_full_list_still_reaches_thirteen`, `::test_the_core_required_tags_are_always_present` | `::test_no_duplicates` | n/a | `::test_every_tag_is_within_etsys_twenty_character_limit` | `::test_never_more_than_thirteen` + existing `test_newduck_publish_seo.py` tag suites (3 of which caught the un-exempted gates) |
| End to end | `::TestThroughThePublicEntryPoint::test_coerce_etsy_tags_applies_the_policy` (christmas out, halloween in, exactly one jeep) | n/a | n/a | n/a | n/a |

**Migrated so far:** all four newduck roles, `etsy_review_helper` (customer_message + review_reply), `review_selection_helper`. **Not yet migrated (still inline `gpt-4o-mini` literals, tracked here rather than as prose per [[feedback_residual_risk_needs_a_tracked_home]]):** thursday ×6, weekly ×3 (one `gpt-4o`), ops ×2, jeepfact ×3, blog, competitor_engine, profit, meme_helper, duck_image_helper, theme_classifier, main_agent. These are unchanged in behaviour — the normalizer is a no-op for classic models — but they still cannot be upgraded without editing each call site. `social_copy` is deliberately staying on gpt-4o-mini (high volume, low stakes, short copy, human-reviewed).


**2026-09-25 follow-up — the model was not the binding constraint (operator: "Which ones should be higher than now?").** Two findings outranked any model change:

1. **The visual brief looked at 4 of 12 selected photos** (`staged_images[:4]`, hard-coded, no override). Twelve get selected and uploaded; eight angles never reached the model. No upgrade recovers detail that was never sent — this is why the production brief said "gold buttons" where a run over a different four said "yellow". Now `_newduck_visual_brief_photo_limit()`, default 8, clamped to [1, 12], `NEWDUCK_VISUAL_BRIEF_PHOTO_LIMIT` to override.
2. **Images were sent at OpenAI `detail: "low"`** — a 512px downsample on the one call whose job is reading small costume details. Now `_newduck_visual_brief_image_detail()`, default `high`, validated against {low, high, auto} so a typo cannot reach the API.

**Measured, same model, isolating the inputs** (gpt-5.5, 13 staged photos): old inputs (4 @ low) and new inputs (8 @ high) both returned 8 features, so this is a refinement rather than a step change — but only the new inputs named the defining feature, **"headless neck opening with tall black collar"** and *"leaving the neck area visibly headless"*, plus the print layer lines. The big jump was the model itself (4 → 8 features); this pass makes it read the structure correctly.

**Determinism repairs.** `listing_copy_hardener` ran at `temperature=0.2`. It is a remove-risky-claims **verdict**, and the house rule is that verdict calls are deterministic — 0.2 is the exact value that made the Thursday qualifier flip run to run. Now 0.0, and the role carries `reasoning_effort: low` so the normalizer cannot promote it back to medium.

**Per-role reasoning effort (`models_version: 2`).** A model per role was not enough: the right effort differs sharply by job. `customer_message` → **high** (refund and replacement offers: lowest volume, highest sensitivity, every one human-reviewed, so depth is free). `classification` and `listing_copy_hardener` → **low** (determinism beats depth for a label or a verdict). `listing_vision`, `review_reply`, `concept_naming` → medium. `DUCK_MODEL_<ROLE>_EFFORT` overrides per role. Roles with no configured effort still defer to the caller's temperature.

**`role=` kwarg on `openai_json` / `openai_json_with_debug`** is now the migration path — one word replaces a hard-coded model string and brings the effort with it. Used to move the Thursday **qualifier and per-candidate renamer** (`_llm_qualify_thursday_candidates`, `_llm_rename_thursday_candidate`, `_llm_duck_names_and_phrases`, `_llm_shorten_titles_for_ducks`) onto `concept_naming`. Those were on gpt-4o-mini via `openai_json`'s *signature default*, which is why grepping for model literals never found them — and they are the judgment the operator called "bad as of late", the reason the override queue (Surface 74) exists. A call site that passes no `role` behaves exactly as before, asserted by `::test_no_role_keeps_the_previous_default`.

Added tests: `tests/test_model_registry.py::TestPerRoleReasoningEffort` (5), `::TestRoleKwargOnTheJsonHelpers` (5 — incl. explicit model still wins, and un-migrated callers unchanged), `tests/test_newduck_visual_brief_inputs.py` (10 — photo limit clamping, detail validation, hardener temp 0, Thursday role wiring).

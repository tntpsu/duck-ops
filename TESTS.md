...
**Follow-up (OPEN):** layer-2 slug-merge is still fragile for two *non-Build-Next* scouts proposing the same duck under different cleaned themes (e.g. trend vs competitor_motif). The real fix is to normalize every scout's `theme` to a clean subject before the slug-merge (or dedup the funnel by token coverage, not exact slug). Build-Next's own awareness (layer 3) already protects the operator-facing Promote path; this is the remaining cross-scout hole.
